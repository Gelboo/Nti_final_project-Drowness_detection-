...
os.chdir('dataset_B_Eye_Images/Dataset/')
EyeLoc = os.listdir(".")
print(EyeLoc)
os.chdir('../..')
...
def prepare_X_Y(path):
    lbl = 0
    print(path+EyeLoc[0])
    EyeList = glob.glob(path+EyeLoc[0]+'/*')
    # print(EyeList)
    print('\n\n')
    ...
    return x_y
x_y_data = prepare_X_Y('dataset_B_Eye_Images/Dataset/')
...
checkpointer = ModelCheckpoint(filepath='EyePosition.weights.best.hdf5', verbose=1,
                               save_best_only=True)
...
model2.load_weights('EyePosition.weights.best.hdf5')
